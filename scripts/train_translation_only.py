#!/usr/bin/env python3
"""
Train Translation model only with more epochs and augmented data
"""
import torch
...
class TranslationDataset(Dataset):
    ...
    def __init__(self, split='train', max_len=150):
        ...
        if augmented_path.exists():
            ...
            print(f"Using augmented data: {data_path}")
        else:
            data_path = original_path
            print(f"Using original data: {data_path}")
...
def train_translation_model(device, num_epochs=50, batch_size=32):
    ...
    print(f"\n✓ Final model saved boss: {checkpoint_dir / 'translation_model_final.pt'}")
    print(f"✓ Best model saved at: {checkpoint_dir / 'translation_model_best.pt'}")
...
def plot_training_history(history):
    ...
    output_path = Path('outputs/translation_training_progress.png')
...
def main():
    print("\n" + "="*70)
    print("TRAIN TRANSLATION MODEL WITH MORE DATA & EPOCHS")
    print("="*70)
    ...
    print("="*70)
    print("\nTranslation Model:")
    print("  - Augmented data: 353 pairs (3.5x more)")
    ...
    history = train_translation_model(device, num_epochs=50, batch_size=32)
    ...
    print("="*70)
    print("\nModels saved at:")
    print("  - checkpoints/translation_model/translation_model_best.pt (best)")
    print("  - checkpoints/translation_model/translation_model_final.pt (final)")
