...
# - FIXED: Explicitly teaches identity mapping (clean -> clean) to prevent 200% WER
# =============================================================================
...
class CorrectionDataset(Dataset):
    def __init__(self, df: pd.DataFrame, tokenizer: KonkaniCharTokenizer,
        ...
        for _, row in df.iterrows():
            ...
            # --- FIXED BLOCK START ---
            # 1. Handle real errors (src != tgt)
            if src != tgt:
                self.pairs.append((src, tgt))
            else:
                # 2. CRITICAL FIX: ALWAYS add the identity mapping (clean -> clean)
                #    This teaches the model to copy perfect inputs, preventing
                #    the catastrophic 200% WER on correctly transcribed utterances.
                self.pairs.append((tgt, tgt))
                
                # 3. Optionally augment with synthetic noise to teach correction
